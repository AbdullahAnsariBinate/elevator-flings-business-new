...
export const login = (payload, CB) => async dispatch => {
  ...
  try {
    ...
    if (response?.data?.error) {
      ...
      await _setDataToAsyncStorage(TOKEN, response?.data?.accessToken);
      await getTokenAndSetIntoHeaders(response?.data?.accessToken);
      dispatch({
...
export const guestCheckout = (payload, CB) => async dispatch => {
  ...
  try {
    let response = await post(REGISTER, payload);
    console.log(
      '🚀 ~ file: Auth.action.js ~ line 198 ~ guestCheckout ~ response',
      response,
    );
    if (response?.data?.error) {
      handleError(response?.data?.message || '');
